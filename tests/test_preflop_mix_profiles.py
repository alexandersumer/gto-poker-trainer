...
def test_weak_offsuit_hands_fold_more_as_opens_grow() -> None:
    ...
    assert mix_small.get("fold", 0.0) <= mix_mid.get("fold", 0.0) <= mix_large.get("fold", 0.0)
    assert mix_large.get("fold", 0.0) == pytest.approx(1.0, abs=1e-6)
...
def test_value_hands_prefer_aggressive_actions() -> None:
    ...
    assert mix_large.get("threebet", 0.0) >= 0.7
...
def test_cfr_profile_adjusts_to_open_size() -> None:
    ...
    assert profile_small.get("fold", 0.0) <= profile_large.get("fold", 0.0)
