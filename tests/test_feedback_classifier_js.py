...
def test_feedback_classifier_core_bands():
    cases = [
        ...
        {
            "id": "yellow_pot8",
            "args": {"feedback": {}, "node": {"pot_bb": 8}, "evLossRaw": 0.18},
        },
        {
            "id": "red_pot10",
            "args": {"feedback": {}, "node": {"pot_bb": 10}, "evLossRaw": 0.6},
        },
        {
            "id": "yellow_pot_unknown",
            "args": {"feedback": {}, "node": {}, "evLossRaw": 0.3},
        },
    ]
    ...
    # Threshold sanity
    assert abs(results["yellow_pot8"]["thresholds"]["greenCutoff"] - 0.032) < 1e-9
    assert abs(results["yellow_pot8"]["thresholds"]["yellowUpper"] - 0.4) < 1e-9
    assert abs(results["yellow_pot_unknown"]["thresholds"]["yellowUpper"] - 0.35) < 1e-9
