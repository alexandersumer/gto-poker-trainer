...
from ..dynamic.equity import estimate_equity
...
class _TextualPresenter(Presenter):
    ...
    def prompt_choice(self, n: int) -> int:  # noqa: D401
...
class TrainerApp(App[None]):
    ...
    def show_step_feedback(self, _node: Node, chosen: Option, best: Option) -> None:
        ...
        if correct:
            ...
            lines.append(
                f"You chose: {chosen.key} (EV {chosen.ev:.2f} bb) → EV lost: [red]{ev_loss:.2f} bb[/]"
            )
        if chosen.why:
